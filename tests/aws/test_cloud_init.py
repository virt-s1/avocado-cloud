...
class CloudInit(Test):
    ...
    def test_check_username(self):
        ...
        bugzilla_id: n/a
        customer_case_id: n/a
        maintainer: xiliang
        case_priority: 0
        case_component: cloud-init
        key_steps:
        1. Launch an instance on AWS EC2.
        2. Connect the instance via ssh with user:ec2-user.
        pass_criteria: ec2-user exists and can access instance using it.
        '''
    ...
    def test_check_userdata(self):
        ...
        pass_criteria: The passed userdata (time.log) should exist and can be edit and remove.
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
        user_name = self.params.get('ssh_user')
    ...
    def test_check_config_ipv6(self):
        ...
        pass_criteria: The IPv6 address shows in NIC and can be connected.
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
    ...
    def test_check_output_isexist(self):
        ...
        pass_criteria: The cloud-init log /var/log/cloud-init-output.log exists, and its stdout and stderr is redirected to this log.
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
    ...
    def test_check_metadata(self):
        ...
        pass_criteria: Correct instance type is returned.
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
    ...
    def test_check_cloudinit_log_traceback(self):
        ...
        pass_criteria: There isn't traceback log in cloudinit logs.
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
    ...
    def test_check_cloudinit_log_unexpected(self):
        ...
        1. Launch an instance on AWS EC2.
        2. Check the cloud-init logs: /var/log/cloud-init.log or /var/log/cloud-init-output.log.
        pass_criteria: There isn't unexpected error in cloudinit logs.
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
    ...
    def test_check_cloudinit_log_critical(self):
        ...
        1. Launch an instance on AWS EC2.
        2. Check the cloud-init logs: /var/log/cloud-init.log or /var/log/cloud-init-output.log.
        pass_criteria: There isn't CRITICAL error in cloudinit logs.
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
    ...
    def test_check_cloudinit_log_warn(self):
        ...
        1. Launch an instance on AWS EC2.
        2. Check the cloud-init logs: /var/log/cloud-init.log or /var/log/cloud-init-output.log.
        pass_criteria: There isn't WARNING in cloudinit logs.
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
    ...
    def test_check_cloudinit_log_error(self):
        ...
        1. Launch an instance on AWS EC2.
        2. Check the cloud-init logs: /var/log/cloud-init.log or /var/log/cloud-init-output.log.
        pass_criteria: There isn't error log in cloudinit logs.
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
    ...
    def test_check_cloudinit_service_status(self):
        ...
        pass_criteria: The 4 cloud-init services status should be "active".
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
    ...
    def test_check_cloudinit_log_imdsv2(self):
        ...
        2. Check the cloud-init fetched Ec2 IMDSv2 API Token and got metadata using the token by commands:
           ...
           "sudo cat /var/log/cloud-init.log"
        pass_criteria: There are outputs link these: "DataSourceEc2.py[DEBUG]: Fetching Ec2 IMDSv2 API Token", "X-aws-ec2-metadata-token", and no error or unexpected message.
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
    ...
    def test_check_cloudinit_ds_identify_found(self):
        ...
        pass_criteria: There are keywords like these: "ds-identify _RET=found".
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
    ...
    def test_check_lineoverwrite(self):
        ...
        This is a specific case of openstack, because the cloud guest images need to have "NOZEROCONF=yes" in /etc/sysconfig/network 
        so that it works well as an openstack guest. (Bug 983611 - Cloud guest images needs to have "NOZEROCONF=yes" in /etc/sysconfig/network)
        cloud-init removed user configuration in
        ...
        re-provision. linked case RHEL-152730
        polarion_id:
        ...
        pass_criteria: "NETWORKING_IPV6=no" and "NOZEROCONF=yes" should be in the network configuration.
        '''
        self.session.connect(timeout=self.ssh_wait_timeout)
        utils_lib.run_cmd(self,
                    'uname -r',
