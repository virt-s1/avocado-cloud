...
def get_remote_public_key(session, public_key="rsa"):
    ...
    if has_dsa_keypair and public_key == "dsa":
        loggin.info('DSA keypair found on %s, using it', session)
        public_key_path = dsa_public_key_path
...
def setup_ssh_key(hostname, user, password, port=22, client_user=None):
    ...
    :type client_user: str
    """
    loggin.debug('Performing SSH key setup on %s:%d as %s.' %
              (hostname, port, user))
    ...
    try:
        ...
        loggin.debug('SSH key setup complete.')

    except Exception:
        loggin.debug('SSH key setup has failed.')
...
def setup_remote_ssh_key(hostname1, user1, password1,
    ...
    try:
        ...
        loggin.debug('SSH key setup on %s complete.', session2)
    except Exception as err:
        loggin.debug('SSH key setup has failed: %s', err)
        try:
            session1.close()
            session2.close()
        except Exception:
...
def setup_remote_known_hosts_file(client_ip, server_ip,
    ...
    """
    loggin.debug('Performing known_hosts file setup on %s from %s.' %
              (server_ip, client_ip))
    abs_path = ""
    try:
        abs_path = path.find_command("ssh-keyscan")
    except path.CmdNotFoundError as err:
        loggin.debug("Failed to find the command: %s", err)
        return None
