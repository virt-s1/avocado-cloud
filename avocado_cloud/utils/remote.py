...
def remote_login(client,
    ...
    :param authentication: The PreferredAuthentications. Value can be:
    ...
    if client == "ssh":
        if not bind_ip:
            cmd = ("ssh %s -o UserKnownHostsFile=/dev/null "
                   ...
                   "-o PreferredAuthentications=%s -p %s %s@%s" %
                   (verbose, authentication, port, username, host))
        else:
            cmd = ("ssh %s -o UserKnownHostsFile=/dev/null "
                   "-o StrictHostKeyChecking=no "
                   "-o PreferredAuthentications=%s -p %s -b %s %s@%s" %
                   (verbose, authentication, port, bind_ip, username, host))
    elif client == "telnet":
...
def wait_for_login(client,
    ...
    :return: A ShellSession object.
    """
    logging.debug("Attempting to log into %s:%s using %s (timeout %ds)", host,
                  port, client, timeout)
    end_time = time.time() + timeout
    verbose = False
    while time.time() < end_time:
        try:
            return remote_login(client,
                                host,
                                port,
                                username,
                                password,
...
def _remote_scp(session,
    ...
    while True:
        try:
            match, text = session.read_until_last_line_matches(
                patterns=patterns, timeout=timeout, internal_timeout=0.5)
            if match == 0:  # "Are you sure you want to continue connecting"
                logging.debug("Got 'Are you sure...', sending 'yes'")
                session.sendline("yes")
                continue
                ...
                authentication_done = True
                # the current pattern should be matched only once
                patterns = [
...
def scp_to_remote(host,
    ...
    command = (
        ...
        "-o StrictHostKeyChecking=no "
        "-o PreferredAuthentications=%s -r %s "
        "-P %s %s %s@\[%s\]:%s" %
        (authentication, limit, port, local_path, username, host, remote_path))
    password_list = []
    password_list.append(password)
    return remote_scp(command, password_list, log_filename, timeout)


def scp_from_remote(host,
                    port,
                    username,
                    password,
                    remote_path,
    ...
    command = (
        ...
        "-o PreferredAuthentications=%s -r %s "
        "-P %s %s@\[%s\]:%s %s" %
        (authentication, limit, port, username, host, remote_path, local_path))
    password_list = []
    password_list.append(password)
    remote_scp(command, password_list, log_filename, timeout)


def scp_between_remotes(src,
                        dst,
                        port,
                        s_passwd,
                        d_passwd,
...
def nc_copy_between_remotes(src,
                            dst,
                            s_port,
                            s_passwd,
                            d_passwd,
...
def throughput_transfer(func):
    ...
    def transfer(*args, **kwargs):
        if "from" in func.__name__:
            msg = ("Copy file from %s:%s to %s, " %
                   (args[0], args[5], args[6]))
        else:
            msg = ("Copy file from %s to %s:%s, " %
...
def copy_files_to(address,
    ...
    if client == "scp":
        ...
        raise exceptions.TestError("No such file copy client: '%s', valid"
                                   "values is scp" % client)


@throughput_transfer
def copy_files_from(address,
                    client,
                    username,
                    password,
                    port,
...
class Remote_Package(object):
    ...
    def pull_file(self, local_path, timeout=600):
        ...
        """
        logging.debug("Pull remote: '%s' to local: '%s'." %
                      (self.remote_path, local_path))
        copy_files_from(self.address,
                        self.cp_client,
                        self.username,
                        self.password,
                        self.cp_port,
